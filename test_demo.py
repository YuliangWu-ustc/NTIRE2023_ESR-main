...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        from models.team21_RRRN import rrrb_deploynet
        name, data_range = f"{model_id:02}_RRRN", 1.
        model_path = 'model_zoo/tram21_rrrn.pth'
        model = rrrb_deploynet()
...
def select_dataset(data_dir, mode):
    if mode == "test":
        path = [
            (
                os.path.join(data_dir, f"lq/{i:04}.png"),
                os.path.join(data_dir, f"hq/{i:04}.png")
...
def run(model, model_name, data_range, tile, logger, device, args, mode="test"):
    ...
    logger.info("{:>16s} : {:<.3f} [M]".format("ave_psnr", results[f"{mode}_ave_psnr"]))  # psnr
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", default="./validation", type=str)
    parser.add_argument("--save_dir", default="./results", type=str)
    parser.add_argument("--model_id", default=21, type=int)
    parser.add_argument("--include_test",default=False,action="store_true", help="Inference on the DIV2K test set")
    parser.add_argument("--hybrid_test",default=False, action="store_true", help="Hybrid test on DIV2K and LSDIR test set")
