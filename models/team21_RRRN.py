...
import basicsr.models.archs.repblock as repblock
import basicsr.models.archs.dbb_block as dbbblock
import basicsr.models.archs.RRRB_block as RRRBblock
import torch
import cv2
import sys
sys.path.append(r'C:\Users\LBH666\Downloads\NTIRE2023_ESR-main\NTIRE2023_ESR-main')
...
class CAESA(nn.Module):
    ...
    def __init__(self, n_feats, DW_Expand=1):
...
class rrrb_deploynet(nn.Module):
    def __init__(self,
                in_channels=3,
                out_channels=3,
                feature_channels=58,
                upscale=4,):
        super(rrrb_deploynet, self).__init__()
...
if __name__ == '__main__':
    x = torch.load('aaa.pth')
    net = rrrb_deploynet()
    model_path = r'C:\Users\LBH666\Downloads\NTIRE2023_ESR-main\NTIRE2023_ESR-main\model_zoo\tram21_rrrn.pth'
    load_net = torch.load(model_path, map_location=lambda storage, loc: storage)['params']
    net.load_state_dict(load_net, strict=False)
    y = net(x['lq'])
    
    y = util.tensor2img_tmp(y)
